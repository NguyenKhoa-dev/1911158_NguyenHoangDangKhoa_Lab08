--CREATE DATABASE [RestaurantManagementLab8]
USE [RestaurantManagementLab8]
GO
...
AS
SELECT RoleID FROM Account A, RoleAccount B WHERE A.AccountName = B.AccountName AND A.AccountName = @AccountName AND RoleID = 1 AND Actived = 1
go	
...
CREATE PROCEDURE BillDetailsInfo
(
	@BillID int
)
As
Begin
	SELECT A.FoodID, B.Name, Quantity, B.Price, B.Price * Quantity as Summary FROM BillDetails A, Food B WHERE A.FoodID = B.ID AND A.InvoiceID = @BillID
End
go
...
--update Bills set CheckoutDate = '11/16/2021' where ID = 1
select * from Account
select * from RoleAccount
